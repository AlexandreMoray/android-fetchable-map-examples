...
class PopupMapActivity : AppCompatActivity() {
    private val MARKER_SELECTED_ICON = "JAWG_ICON"
    private val MARKER_ICON = "MARKER_ICON"
    ...
    // Set specified marker layout to classic.
    private fun setClassicMarker(symbol: Symbol) {
        symbol.iconImage = MARKER_ICON
        symbol.iconSize = 0.24f
        symbolManager?.update(symbol)
    }

    // Set specified marker layout to selected.
    private fun setSelectedMarker(touchedSymbol: Symbol) {
        touchedSymbol.iconImage = MARKER_SELECTED_ICON
        touchedSymbol.iconSize = 0.25f
        symbolManager?.update(touchedSymbol)

        if(this.lastSymbol != null) {
            setClassicMarker(this.lastSymbol!!)
        }
        this.lastSymbol = touchedSymbol
    }
